...
struct TopkCompressorCountMin : public CompressorBase {
    uint64_t k = 1_Mi;
    uint64_t sketch_rows = 2;
    uint64_t sketch_columns = 1_Mi;

    TopkCompressorCountMin(std::string&& type_name, std::string&& desc) : CompressorBase(std::move(type_name), std::move(desc)) {
