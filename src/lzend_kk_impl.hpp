...
void lzend_kk_compress(In begin, In const& end, Out out, size_t const block_size, pm::Result& result) {
    ...
    // fully read file into RAM (TODO: read window by window)
    std::string window;
    std::copy(begin, end, std::back_inserter(window));
    
    Index const window_size = window.length();
    ...
    for(Index m = 0; m < window_size; m++) {
        if constexpr(DEBUG) std::cout << "m=" << m << ", s[m]=" << display(window[m]) << std::endl;
