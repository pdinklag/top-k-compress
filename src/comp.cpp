    ...
    if(app) {
        if(!app.args().empty()) {
            ...
            if(options.decompress) {
                ...
                tdc::io::FileOutputStream fos(options.output);
                if(options.huffman) {
                    top_k_compress_huff(fis.begin(), fis.end(), tdc::io::bitwise_output_to(fos), options.raw, options.k, options.window, options.sketch_rows, options.sketch_columns);
                } else {
                    top_k_compress_binary(fis.begin(), fis.end(), tdc::io::bitwise_output_to(fos), options.raw, options.k, options.window, options.sketch_rows, options.sketch_columns);
                }
            }
        } else {
