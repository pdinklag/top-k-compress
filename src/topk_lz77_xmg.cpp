...
#include <topk_prefixes_misra_gries2.hpp>

using Topk = TopKPrefixesMisraGries2<>;

struct Compressor : public TopkCompressor {
    uint64_t window = 8;
    unsigned int threshold = 2;

    Compressor() : TopkCompressor("topk-lz77-xmg", "Factorizes (small) windows to use as relevant strings in a Misra-Gries top-k data structure.") {
